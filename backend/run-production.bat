@echo off
REM Batch file to run the production version of the P4 Backend locally
...
echo Running P4 Backend in Production Mode...
echo.
...
if exist "mvnw.cmd" (
    ...
    call mvnw.cmd spring-boot:run -Dspring-boot.run.profiles=prod -Dspring-boot.run.main-class=com.p4.backend.P4BackendApplication
) else (
    echo Using mvn command...
    call mvn spring-boot:run -Dspring-boot.run.profiles=prod -Dspring-boot.run.main-class=com.p4.backend.P4BackendApplication
)
