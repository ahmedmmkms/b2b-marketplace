...
def test_fulltext_search():
    ...
    for i, product_data in enumerate(products_data):
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                print(f"   SUCCESS: Product {i+1} created successfully with ID: {product_id}")
            else:
                print(f"   ERROR: Failed to create product {i+1}. Status: {response.status_code}, Response: {response.text}")
                return False
        except Exception as e:
            print(f"   ERROR: Error creating product {i+1}: {str(e)}")
            return False
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if isinstance(results, list):
                print(f"   SUCCESS: Full-text search for 'wireless headphones' successful, found {len(results)} products")
                ...
                if found_headphones:
                    print("   SUCCESS: Correctly found headphones in search results")
                else:
                    print("   WARNING: Headphones product not found in search results")
            else:
                print(f"   ERROR: Expected a list of products, got: {type(results)}")
                return False
        else:
            print(f"   ERROR: Full-text search for 'wireless headphones' failed. Status: {response.status_code}, Response: {response.text}")
            return False
    except Exception as e:
        print(f"   ERROR: Error searching for 'wireless headphones': {str(e)}")
        return False
    ...
    try:
        ...
        if response.status_code == 200:
            results = response.json()
            if isinstance(results, list):
                print(f"   SUCCESS: Full-text search for '4k tv' successful, found {len(results)} products")
                # Verify that the TV product is in the results
                found_tv = any('tv' in prod.get('name', '').lower() for prod in results)
                if found_tv:
                    print("   SUCCESS: Correctly found TV in search results")
                else:
                    print("   WARNING: TV product not found in search results")
            else:
                print(f"   ERROR: Expected a list of products, got: {type(results)}")
                return False
        else:
            print(f"   ERROR: Full-text search for '4k tv' failed. Status: {response.status_code}, Response: {response.text}")
            return False
    except Exception as e:
        print(f"   ERROR: Error searching for '4k tv': {str(e)}")
        return False
    ...
    try:
        ...
        if response.status_code == 200:
            results = response.json()
            if isinstance(results, list):
                print(f"   SUCCESS: Full-text search for 'dslr camera' successful, found {len(results)} products")
                # Verify that the camera product is in the results
                found_camera = any('camera' in prod.get('name', '').lower() for prod in results)
                if found_camera:
                    print("   SUCCESS: Correctly found camera in search results")
                else:
                    print("   WARNING: Camera product not found in search results")
            else:
                print(f"   ERROR: Expected a list of products, got: {type(results)}")
                return False
        else:
            print(f"   ERROR: Full-text search for 'dslr camera' failed. Status: {response.status_code}, Response: {response.text}")
            return False
    except Exception as e:
        print(f"   ERROR: Error searching for 'dslr camera': {str(e)}")
        return False
    ...
    try:
        ...
        if response.status_code == 200:
            results = response.json()
            if isinstance(results, list):
                print(f"   SUCCESS: Full-text search for 'noise cancellation' successful, found {len(results)} products")
                ...
                if found_relevant:
                    print("   SUCCESS: Correctly found product with both 'noise' and 'cancellation' terms")
                else:
                    print("   WARNING: Product with 'noise' and 'cancellation' terms not found in search results")
            else:
                print(f"   ERROR: Expected a list of products, got: {type(results)}")
                return False
        else:
            print(f"   ERROR: Full-text search for 'noise cancellation' failed. Status: {response.status_code}, Response: {response.text}")
            return False
    except Exception as e:
        print(f"   ERROR: Error searching for 'noise cancellation': {str(e)}")
        return False
    ...
    try:
        ...
        if response.status_code == 200:
            results = response.json()
            if isinstance(results, list):
                print(f"   SUCCESS: Full-text search for 'stunning 4K resolution' successful, found {len(results)} products")
                ...
                if found_relevant:
                    print("   SUCCESS: Correctly found product with 'stunning' and '4K' terms in description")
                else:
                    print("   WARNING: Product with 'stunning' and '4K' terms in description not found in search results")
            else:
                print(f"   ERROR: Expected a list of products, got: {type(results)}")
                return False
        else:
            print(f"   ERROR: Full-text search for 'stunning 4K resolution' failed. Status: {response.status_code}, Response: {response.text}")
            return False
    except Exception as e:
        print(f"   ERROR: Error searching for 'stunning 4K resolution': {str(e)}")
        return False
    ...
    try:
        ...
        if response.status_code == 200:
            results = response.json()
            if isinstance(results, list):
                print(f"   SUCCESS: Full-text search for brand 'AudioPro' successful, found {len(results)} products")
                ...
                if found_brand:
                    print("   SUCCESS: Correctly found product with 'AudioPro' brand")
                else:
                    print("   WARNING: Product with 'AudioPro' brand not found in search results")
            else:
                print(f"   ERROR: Expected a list of products, got: {type(results)}")
                return False
        else:
            print(f"   ERROR: Full-text search for brand 'AudioPro' failed. Status: {response.status_code}, Response: {response.text}")
            return False
    except Exception as e:
        print(f"   ERROR: Error searching for brand 'AudioPro': {str(e)}")
        return False
    ...
    try:
        ...
        if response.status_code == 200:
            results = response.json()
            if isinstance(results, list):
                print(f"   SUCCESS: Full-text search for 'electronics audio' successful, found {len(results)} products")
                ...
                if found_electronics:
                    print("   SUCCESS: Correctly found product in electronics-audio category")
                else:
                    print("   WARNING: Product in electronics-audio category not found in search results")
            else:
                print(f"   ERROR: Expected a list of products, got: {type(results)}")
                return False
        else:
            print(f"   ERROR: Full-text search for 'electronics audio' failed. Status: {response.status_code}, Response: {response.text}")
            return False
    except Exception as e:
        print(f"   ERROR: Error searching for 'electronics audio': {str(e)}")
        return False
    ...
    try:
        ...
        if response.status_code == 200:
            results = response.json()
            if isinstance(results, list):
                print(f"   SUCCESS: Full-text search for 'professional DSLR camera with 24MP sensor' successful, found {len(results)} products")
                ...
                if found_camera:
                    print("   SUCCESS: Correctly found camera with 'professional', 'dslr', and '24mp' terms")
                else:
                    print("   WARNING: Camera with 'professional', 'dslr', and '24mp' terms not found in search results")
            else:
                print(f"   ERROR: Expected a list of products, got: {type(results)}")
                return False
        else:
            print(f"   ERROR: Full-text search for 'professional DSLR camera with 24MP sensor' failed. Status: {response.status_code}, Response: {response.text}")
            return False
    except Exception as e:
        print(f"   ERROR: Error searching for 'professional DSLR camera with 24MP sensor': {str(e)}")
        return False
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if isinstance(results, list) and len(results) > 0:
                print(f"   SUCCESS: Full-text search for 'headphones' successful, found {len(results)} products")
                ...
                if 'headphones' in first_result.get('name', '').lower() or 'headphones' in first_result.get('description', '').lower():
                    print("   SUCCESS: Most relevant result (containing 'headphones') appears first")
                else:
                    print("   WARNING: Most relevant result (containing 'headphones') does not appear first")
            else:
                print(f"   ERROR: Expected a list of products, got: {type(results)}")
                return False
        else:
            print(f"   ERROR: Full-text search for 'headphones' failed. Status: {response.status_code}, Response: {response.text}")
            return False
    except Exception as e:
        print(f"   ERROR: Error testing search relevance ranking: {str(e)}")
        return False
...
def main():
    ...
    print("="*80)
    if success:
        print("SUCCESS: All tests passed! Task 5.6 implementation is working correctly in production.")
        sys.exit(0)
    else:
        print("ERROR: Some tests failed! Task 5.6 implementation needs review.")
        sys.exit(1)
