# MVP Database Schema (PostgreSQL 15+)

> Scope: **Catalog → RFQ → Quote → Order → Wallet Pay** (feature-flagged exposure)

## Conventions
- IDs are **ULIDs** stored as `char(26)` (upper-case, Crockford base32).
- All tables have: `created_at timestamptz default now()`, `updated_at timestamptz default now()` with trigger.
- Soft-delete only where noted (`deleted_at timestamptz`).
...
## Notes
- Catalog search uses Postgres GIN + trigram on `products.name` and faceting via `attributes` JSONB.
- Payments: MVP prefers **wallet**; `gateway1` can be enabled later behind flags.
- Orders are **one-to-one** with the **accepted quote** (MVP simplification).
