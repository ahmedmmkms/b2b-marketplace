...
def test_vendor_entity():
    ...
    try:
        ...
        if response.status_code == 200 or response.status_code == 201:
            ...
            if vendor_id:
                ...
                print(f"   ✓ Vendor status: {vendor_response.get('data', {}).get('vendorStatus', vendor_response.get('vendorStatus', 'Unknown'))}")
            else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if retrieved_vendor_id == vendor_id:
                print(f"   ✓ Vendor retrieved successfully with ID: {retrieved_vendor_id}")
                print(f"   ✓ Business name: {vendor_data.get('data', {}).get('businessName', vendor_data.get('businessName', 'Unknown'))}")
            else:
                print(f"   ✗ Vendor retrieval failed. Retrieved ID: {retrieved_vendor_id}, Expected: {vendor_id}")
                return False
        else:
            print(f"   ✗ Vendor retrieval failed. Status: {response.status_code}, Response: {response.text}")
            return False
    except Exception as e:
        print(f"   ✗ Error during vendor retrieval: {str(e)}")
        return False
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if updated_status == "APPROVED":
                ...
                print(f"   ✗ Vendor update failed. Status: {updated_status}")
                return False
        else:
            print(f"   ✗ Vendor update failed. Status: {response.status_code}, Response: {response.text}")
            return False
    except Exception as e:
        print(f"   ✗ Error during vendor update: {str(e)}")
        return False
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if vendors:
                ...
                if approved_found:
                    print(f"   ✓ Found vendors with APPROVED status")
                    print(f"   ✓ Total vendors with status APPROVED in response: {len([v for v in vendors if v.get('vendorStatus') == 'APPROVED'])}")
                else:
                    print(f"   ⚠ Could not verify APPROVED status in response, but request was successful")
            else:
                print(f"   ⚠ No vendors returned in search, but request was successful")
        else:
            print(f"   ✗ Vendor search failed. Status: {response.status_code}, Response: {response.text}")
            return False
    except Exception as e:
        print(f"   ✗ Error during vendor search: {str(e)}")
        return False
    ...
    try:
        ...
        response = requests.get(f"{API_URL_BASE}/api/vendors/{vendor_id}", headers=HEADERS)
        if response.status_code == 200:
            print(f"   ✓ Vendor still accessible after update operations")
        else:
            print(f"   ✗ Vendor not accessible after update. Status: {response.status_code}, Response: {response.text}")
            return False
    except Exception as e:
        print(f"   ✗ Error during vendor existence check: {str(e)}")
        return False

    print("\n✓ All vendor entity and repository tests passed!")
    return True
