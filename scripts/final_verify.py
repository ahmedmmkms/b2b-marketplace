...
def load_db_config():
    ...
    username = os.getenv('DB_USERNAME') or result.username
    if not username:
        raise ValueError("Database username not provided; set DB_USERNAME or include credentials in DB_URL")

    password = os.getenv('DB_PASSWORD') or result.password
    if not password:
        raise ValueError("Database password not provided; set DB_PASSWORD or include credentials in DB_URL")
