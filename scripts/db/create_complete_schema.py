...
try:
    ...
    print("Ensured uuid-ossp extension is installed")
    ...
    $$ LANGUAGE plpgsql;
    """)
    print("Ensured ULID generation function is created")
    
    # Drop all tables if they exist (safeguard)
    tables_to_drop = [
        'idempotency_keys', 'audit_log', 'credit_note_lines', 'credit_notes',
        'invoice_lines', 'invoices', 'sequence_registry', 'tax_registrations',
        'loyalty_transactions', 'account_tiers', 'rewards', 'tiers', 'loyalty_programs',
        'wallet_transactions', 'wallets', 'credit_limits', 'payments', 'order_lines',
        'orders', 'quote_lines', 'quotes', 'rfq_lines', 'rfqs', 'product_media',
        'media_assets', 'products', 'product_attribute_values', 'product_attributes',
        'vendors', 'role_permissions', 'user_roles', 'users', 'roles', 'permissions', 'accounts', 'feature_flags'
    ]
